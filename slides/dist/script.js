...
require.register("svg.js/dist/svg.js", function(exports, require, module) {
  require = __makeRelativeRequire(require, {}, "svg.js");
  (function() {
    /*!
...
require.alias("svg.js/dist/svg.js", "svg.js");})();
...
function controller() {
  ...
  document.getElementById("bGeneration").addEventListener("click", function () {
    ...
    var l = document.getElementById("autospec").getElementsByTagName("textarea")[0].value;
    /*
...
function draw(automata, container) {
  ...
  var radius = 18;
  var offset = Math.sqrt(radius);

  var svg = SVG(container);
  svg.node.setAttribute("preserveAspectRatio", "xMidYMid meet");
  // svg.width("100%").height("auto");

  automata.transitions.forEach(function (t) {
    t.line = svg.line(t.source.x, t.source.y, t.target.x, t.target.y).stroke({ color: "#000", width: 2 }).on("mouseover", function () {
    ...
    t.text = svg.plain(t.symbol.name).place((t.source.x + t.target.x) / 2, (t.source.y + t.target.y) / 2);
  });

  automata.states.forEach(function (s) {
    s.circle = svg.circle().radius(radius).attr({ cx: s.x, cy: s.y }).fill("#fff").stroke({ color: "#000", width: 2 }).click(function () {
      return log.info(s);
    });
    s.text = svg.plain(s.name).place(s.x, s.y + offset);
  });
}
});
...
;require.register("lib/automata.js", function(exports, require, module) {
"use strict";
...
require.register("lib/index.js", function(exports, require, module) {
"use strict";
